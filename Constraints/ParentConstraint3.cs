namespace EgoCS
{
    ...
    public class ParentConstraint< C1, C2, C3, CS1 > : ParentConstraint, IEnumerable< (EgoComponent, C1, C2, C3, CS1) >
        where C1 : Component
        where C2 : Component
        where C3 : Component
        where CS1 : Constraint, new()
    {
        ...
        IEnumerator< (EgoComponent, C1, C2, C3, CS1) > IEnumerable< (EgoComponent, C1, C2, C3, CS1) >.GetEnumerator()
        {
            var lookup = GetLookup( rootBundles );
            foreach( var kvp in lookup )
            {
                currentEgoComponent = kvp.Key;
                var bundle = kvp.Value as Bundle< C1, C2, C3 >;
